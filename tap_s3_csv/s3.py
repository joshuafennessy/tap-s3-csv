...
def sampling_gz_file(table_spec, s3_bucket, s3_path, file_handle, sample_rate, extension, config):
...
def sample_file(table_spec, s3_bucket, s3_path, file_handle, sample_rate, extension, config):
    ...
    if extension in ["csv", "txt"]:
        ...
        return csv_records
    if extension == "gz":
        return sampling_gz_file(table_spec, s3_bucket, s3_path, file_handle, sample_rate, extension, config)
    if extension == "jsonl":
        # If file object read from s3 bucket file else use extracted file object from zip or gz
